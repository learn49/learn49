...
import { Offers } from '@/modules/offers/offers.entity';
...
@Injectable()
export class WebhookTesteService {
  constructor(
  ...
  private async createEnrolment({
    ...
    if (isEnrolled) {
      isEnrolled.transactionId = transactionId;
      await this.enrollmentRepository.save(isEnrolled);
      return;
    }
